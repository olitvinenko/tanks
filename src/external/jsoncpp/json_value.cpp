...
#define JSON_ASSERT_UNREACHABLE {printf("JSON_ASSERT_UNREACHABLE %s\n", __FUNCTION__);}//assert( false );
...
public:
                                       ...
                                       unsigned int length = unknown )
   {
      ...
      if ( !value  ||  value[0] == 0 )
      {
         char *newString = static_cast<char *>( malloc( 1 ) );
         newString[0] = 0;
         return newString;
      }
...
const Value& Value::GetValueByPath(const std::string &_path) const
{
    const Json::Value* val = this;
    std::vector<std::string> elems = split(_path, ',');
